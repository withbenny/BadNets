...
def save_generated_images(ddpm, device, save_dir, times, n_sample=1000, size=(1, 28, 28)):
    ...
    ddpm.eval()
    with torch.no_grad():
        x_gen, _ = ddpm.sample(n_sample, size, device, guide_w=2)
        # Assuming labels are generated in a cyclic manner [0, 1, 2, ..., 9],
        # repeat this cycle as needed to match the number of samples.
        labels = torch.arange(0, 10).repeat(n_sample // 10 + 1)[:n_sample].numpy()
